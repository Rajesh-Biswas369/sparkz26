...
interface AdminGodRegistrationPermitProps {
  ...
  onToggleEventRegistration: () => void;
}

export default function AdminGodRegistrationPermit({ allowRegistration, onToggle, allowEventRegistration, onToggleEventRegistration }: AdminGodRegistrationPermitProps) {
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="bg-white/5 border border-white/10 rounded-xl p-6 mb-8 flex flex-col md:flex-row items-center justify-between gap-6 shadow-[0_0_15px_rgba(0,0,0,0.5)]">
        <div>
          ...
          <p className="text-gray-400 text-sm font-['Inter',sans-serif]">
            Turn this on to allow new students to create an account. If turned off, the signup page will be closed.
          </p>
        </div>
      ...
      <div className="bg-white/5 border border-white/10 rounded-xl p-6 mb-8 flex flex-col md:flex-row items-center justify-between gap-6 shadow-[0_0_15px_rgba(0,0,0,0.5)]">
        <div>
          ...
          <p className="text-gray-400 text-sm font-['Inter',sans-serif]">
            Turn this on to allow students to register for specific events (e.g., Singing, Dance). If turned off, event registration forms will be locked.
          </p>
        </div>
        
        <button
          onClick={onToggleEventRegistration}
          className={`flex items-center gap-3 px-6 py-3 rounded-xl border transition-all duration-300 ${
        ...
        >
          {allowEventRegistration ? <ToggleRight size={28} /> : <ToggleLeft size={28} />}
          <span className="font-['Orbitron',sans-serif] font-bold tracking-widest uppercase text-sm">
            {allowEventRegistration ? 'EVENTS ON' : 'EVENTS OFF'}
          </span>
        </button>
      </div>
    </div>
